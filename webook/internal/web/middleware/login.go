...
func (l *LoginMiddlewareBuilder) Build() gin.HandlerFunc {
	//用Go的方式编码解码  需要先注册一下，提前解析一下time的元数据（time中有那些字段）否则会报错
...
func (l *LoginMiddlewareBuilder) BuildV1() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		// updateTime是有的，做断言一下是否是int64，如果不是可能被篡改过
		updateTimeVal, ok := updateTime.(int64)
		if !ok {
			ctx.AbortWithStatus(http.StatusInternalServerError)
			return
		}
		if now-updateTimeVal > 10*1000 { //当是超过10s的请求之后就会更新update_time 乘以1000是因为是毫秒
			sess.Set("update_time", now)
			sess.Save()
			return
		}
